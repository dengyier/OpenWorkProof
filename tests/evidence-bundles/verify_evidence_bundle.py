...
def _verify_v05_envelope(bundle_path: str) -> bool:
    """Materialize and verify one closed verification-integrity v0.5 package."""
    from openworkproof.delivery_package import (
        DeliveryPackageError,
        verify_delivery_package,
    )
    ...
    if type(metadata) is not dict or set(metadata) != expected_metadata:
        raise DeliveryPackageError("v0.5 envelope metadata is not closed")
    if (
        metadata["demo_owner"] != "OpenWorkProof"
        or metadata["upstream_adoption"] != "not_evidenced"
        or metadata["customer_case"] != "not_evidenced"
        or metadata["commercial_validation"] != "not_evidenced"
        or metadata["blind_population_status"] != "capture_failed"
        or metadata["rot_control_status"] != "mismatched"
        or metadata["repaired_decision"] != "VERIFIED"
        or metadata["full_offline_replay"] is not True
    ):
        raise DeliveryPackageError("v0.5 envelope boundary metadata is invalid")
    if type(envelope["files"]) is not list:
        raise DeliveryPackageError("v0.5 envelope files are invalid")
    paths = tuple(
        item.get("path") for item in envelope["files"] if type(item) is dict
    )
    if (
        ...
        raise DeliveryPackageError("v0.5 envelope paths are not closed")
    with tempfile.TemporaryDirectory(prefix="openworkproof-v05-") as temporary:
        root = Path(temporary)
        for item in envelope["files"]:
            if set(item) != {"path", "sha256", "payload_b64"}:
                raise DeliveryPackageError(
                    "v0.5 envelope file fields are invalid"
                )
            relative = item["path"]
            if (
                type(relative) is not str
                ...
                raise DeliveryPackageError("v0.5 envelope path is unsafe")
            try:
                payload = base64.b64decode(item["payload_b64"], validate=True)
            except (TypeError, ValueError) as error:
                raise DeliveryPackageError(
                    "v0.5 envelope payload is invalid"
                ) from error
            if hashlib.sha256(payload).hexdigest() != item["sha256"]:
                raise DeliveryPackageError("v0.5 envelope file hash mismatch")
            target = root.joinpath(*relative.split("/"))
            target.parent.mkdir(parents=True, exist_ok=True)
            target.write_bytes(payload)
        result = verify_delivery_package(root)
    if (
        ...
        raise DeliveryPackageError("v0.5 replay conclusion is invalid")
    print(f"\n{'=' * 60}")
    print("✅ VERIFICATION PASSED")
    print(f"   Case: {metadata['issue_source']}")
    print("   v0.5 integrity: blind capture_failed -> rot mismatched -> repaired VERIFIED")
    print(f"   Current decision: {result.current_decision}")
    print(f"   Current readiness: {result.settlement_readiness}")
    print(f"   Manifest digest: {result.manifest_digest}")
    print(f"{'=' * 60}\n")
...
def verify_bundle(bundle_path: str) -> bool:
    ...
    if schema == "openworkproof/delivery-package-envelope/0.5":
        return _verify_v05_envelope(bundle_path)
