"""Generate the independent companion (0.1) schema registry.
...
with ``rmtree`` before replacing it.
"""
...
__all__ = [
    ...
    "OBJECT_PATHS",
    "SCHEMA_FACTORIES",
    "generated_companion_files",
    "generate_companion_schemas",
]
...
_STAGE_PREFIX = ".openworkproof-companion-stage-"
_BACKUP_PREFIX = ".openworkproof-companion-backup-"


class CompanionSchemaCleanupError(RuntimeError):
...
def generated_companion_files() -> dict[str, bytes]:
    """Return the canonical registry and every companion schema, deterministically."""
...
def _verify_generated_files(files: dict[str, bytes]) -> None:
    if set(files) != _FILENAMES:
        raise RuntimeError("companion generated file set is invalid")
    for filename, content in files.items():
        _parse_canonical_json(
            content,
            error_message="companion schema is not canonical",
        )
    registry = _parse_canonical_json(
        files[_REGISTRY_FILENAME],
        error_message="companion registry is not canonical",
    )
    expected = {
        "schema_version": _REGISTRY_SCHEMA_VERSION,
        "document_version": COMPANION_VERSION,
    ...
    if registry != expected:
        raise RuntimeError("companion registry is inconsistent")


# --------------------------------------------------------------------------- #
# transaction helpers (local, companion-specific prefixes and file set)
# --------------------------------------------------------------------------- #
...
def _preflight_target(directory: Path) -> tuple[Path, tuple[Path, ...]]:
    ...
    if requested_target.is_symlink():
        raise ValueError("companion schema destination must be a real directory")

    requested_parent = requested_target.parent
    if requested_parent.is_symlink():
        raise ValueError("companion schema destination parent is a symlink")

    missing_names: list[str] = []
    parent = requested_parent
    while not parent.exists():
        if parent.is_symlink() or parent == parent.parent:
            raise ValueError("companion schema destination parent is invalid")
    ...
    if any(
        ...
        raise ValueError("companion schema destination contains unexpected entries")
    ...
    for directory in sorted(candidates, key=lambda value: len(value.parts)):
        if directory.exists():
            if directory.is_symlink() or not directory.is_dir():
                raise ValueError("companion schema destination parent changed")
            continue
        try:
            directory.mkdir()
            ...
            if directory.is_symlink() or not directory.is_dir():
                raise ValueError(
                    "companion schema destination parent changed"
                ) from None
        else:
...
def _validate_open_lock(lock_path: Path, descriptor: int) -> None:
    ...
    try:
        ...
        raise ValueError("companion schema transaction lock is invalid") from error
    if (
        not stat.S_ISREG(opened.st_mode)
        or not stat.S_ISREG(linked.st_mode)
        or (opened.st_dev, opened.st_ino) != (linked.st_dev, linked.st_ino)
    ):
        raise ValueError("companion schema transaction lock is invalid")
    ...
    try:
        for target in ordered:
            ...
            try:
                ...
                raise ValueError(
                    "companion schema transaction lock is invalid"
                ) from error
            try:
...
def _stage_directory(target: Path, files: dict[str, bytes]) -> Path:
    ...
    try:
        ...
        if not _readback_matches(stage, files):
            raise RuntimeError("companion schema staging verification failed")
    except Exception:
        try:
            _remove_schema_directory(stage, frozenset(files))
        except (OSError, RuntimeError) as cleanup_error:
            raise CompanionSchemaCleanupError(
                "companion schema staging failed and its directory could not "
                "be cleaned",
                committed=False,
                stage_paths=(stage,),
            ) from cleanup_error
    ...
    for _ in range(3):
        try:
            if directory.is_symlink():
                raise RuntimeError(
                    "companion schema transaction cleanup target is invalid"
                )
            ...
            if any(
                ...
                raise RuntimeError(
                    "companion schema transaction cleanup set is invalid"
                )
            for entry in entries:
                entry.unlink()
            directory.rmdir()
...
def _recover_target(target: Path) -> None:
    ...
    if not target.exists() and backups:
        if len(backups) != 1:
            raise CompanionSchemaCleanupError(
                "companion schema recovery found ambiguous backups",
                committed=False,
    ...
    if failed_backups or failed_stages:
        raise CompanionSchemaCleanupError(
            "companion schema recovery cleanup failed",
...
def _commit_staged_directories(
    ...
    try:
        for target, stage in targets:
            ...
            try:
                ...
                # Uncertain COMMIT acknowledgement: a filesystem may complete
                # the rename yet still report an error. Read the target back
                # exactly to decide whether the commit actually landed.
                if not _readback_matches(target, files):
                    raise
            installed.add(target)

        # Exact readback after commit: confirm every target carries the
        # expected complete, byte-for-byte file set before acknowledging
        # success. This stays inside the rollback boundary so a tampered or
        # truncated post-replace readback restores the old targets instead of
        # leaving corrupt committed truth.
        for target, _ in targets:
            if not _readback_matches(target, files):
                raise RuntimeError("companion schema commit readback failed")
    except Exception:
        ...
        if rollback_error is not None:
            ...
            raise CompanionSchemaCleanupError(
                "companion schema transaction rollback failed",
                committed=False,
    ...
    if failed_backups:
        raise CompanionSchemaCleanupError(
            "companion schema commit completed but backup cleanup failed",
            committed=True,
            backup_paths=failed_backups,
        ) from first_error


def _write_companion_transaction(
    ...
    if len(resolved) == 2 and (
        ...
        or resolved[0] in resolved[1].parents
        or resolved[1] in resolved[0].parents
    ):
        raise ValueError("companion schema destination and mirror conflict")
    ...
    try:
        ...
        with _locked_targets(resolved):
            ...
            if locked_targets != resolved:
                raise ValueError(
                    "companion schema destination changed before locking"
                )
            ...
            try:
                ...
                if failed_stages:
                    raise CompanionSchemaCleanupError(
                        "companion schema transaction stage cleanup failed",
                        committed=False,
                        stage_paths=failed_stages,
                    ) from first_error
    finally:
...
def generate_companion_schemas(
    destination: Path,
    mirror: Path | None = None,
) -> None:
    """Generate the companion schemas and optional published mirror, safely."""

    files = generated_companion_files()
    _verify_generated_files(files)
    requested = [Path(destination)] + (
        [Path(mirror)] if mirror is not None else []
    )
    _write_companion_transaction(requested, files)


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Generate authoritative companion evidence schemas."
    )
    parser.add_argument("--destination", required=True, type=Path)
    parser.add_argument("--mirror", type=Path)
    arguments = parser.parse_args(argv)
    generate_companion_schemas(arguments.destination, mirror=arguments.mirror)
    return 0
