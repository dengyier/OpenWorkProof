...
- Day0 环境（`/Users/molin/Project/openWorkProof-day0`）+ 候选镜像 + wheelhouse；
...
4. **存档**：签名单 + 哈希清单 + 时间戳存入 `docs/day0/`（版本化提交）。
...
| 独立见证人 | 复核清单、联署 |
| 独立 Acceptor | 环节 1 验收签署（如参与 Day 0 则联署） |

### 4.5 验证与存档机制

- 存档后 `git tag` 冻结点（如 `day0-2026-08-07`）；
- 哈希清单复核脚本（重算比对）；
- 存档目录只读约定 + 提交记录可追溯。
...
M3 环节 3 Day 0 冻结/签署/存档（tag）                    [需 Owner 参与]
